...
export async function getAuthUserByEmail(email) {
  const row = await getDatabase().one('SELECT * FROM users WHERE LOWER(email) = LOWER(?)', [normalizeRegistrationEmail(email)]);
  if (!row) return null;
  return {
    ...publicUser(row),
...
export async function getPendingRegistrationById(id) {
  return pendingRegistration(await getDatabase().one(`
    ${REGISTRATION_SELECT}
...
export async function updatePendingRegistration(id, input) {
  ...
  await db.transaction(async tx => {
    await tx.run('UPDATE volunteers SET name = ?, phone = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [name, phone, current.volunteerId]);
...
export async function approvePendingRegistration(id) {
  ...
  const db = getDatabase();
  await db.transaction(async tx => {
    await tx.run('UPDATE volunteers SET active = 1, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [current.volunteerId]);
    await tx.run(`
      UPDATE users
...
export async function getPublicUserByEmail(email) {
  const user = await getDatabase().one(
    "SELECT * FROM users WHERE LOWER(email) = LOWER(?) AND active = 1 AND approval_status = 'APPROVED'",
    [email?.trim()]
  );
