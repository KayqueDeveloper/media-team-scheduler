  ...
  return (
    <section className="glass-panel admin-exchanges">
      <div className="portal-card-header">
        <h2>
          <BellRing size={19} /> Confirmações de serviço
        </h2>
        <span>
          {active.filter((item) => item.status === 'AWAITING').length} aguardando ·{' '}
          {active.filter((item) => item.status === 'CONFIRMED').length} confirmada(s)
        </span>
      </div>
      {confirmations.length === 0 ? (
        <p className="portal-muted">
          Os lembretes aparecerão aqui a partir de três dias antes de cada domingo.
        </p>
      ) : (
          ...
          {confirmations.map((item) => {
            ...
            return (
              <div className="portal-list-row" key={item.id}>
                <div>
                  <strong>
                    {formatScheduleDate(item.date)} · {getShiftLabel(item.shift)} · {getRoleLabel(item.role)}
                  </strong>
                  <span>
                    {item.volunteerName} · {item.isTrainee ? 'Treinando' : 'Principal'}
                  </span>
                  {item.lastError && (
                    <small className="confirmation-error">Falha no envio: {item.lastError}</small>
                  )}
                ...
                <div className="portal-actions">
                  ...
                  <span className={`portal-tag confirmation-${item.status.toLowerCase()}`}>
                    <StatusIcon size={14} /> {state.label}
                  </span>
                </div>
              </div>
            );
