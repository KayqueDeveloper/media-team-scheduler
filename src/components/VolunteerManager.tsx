...
export const VolunteerManager = ({
  ...
  const toggleVolunteerDetails = (volunteerId) => {
    setExpandedVolunteerIds((current) => {
    ...
    <section className="manager-container glass-panel">
      <div className="manager-toolbar">
        <div>
          ...
          <p>
            Cadastre voluntários, defina restrição de turnos (Manhã/Noite) e ajuste o nível técnico nas 6
            funções.
          </p>
        </div>
      ...
      <div className="volunteers-grid">
        {filteredVolunteers.map((vol) => {
          const detailsId = `volunteer-details-${vol.id}`;
          const detailsExpanded = expandedVolunteerIds.has(String(vol.id));
          return (
            <div key={vol.id} className="volunteer-card">
              <div className="volunteer-card-header">
                <div className="volunteer-avatar-info">
                  <div className="volunteer-avatar">{vol.name.charAt(0)}</div>
                  <div className="volunteer-name-details">
                    <h3>{vol.name}</h3>
                    <p>
                      <Mail size={12} style={{ display: 'inline', marginRight: 4 }} /> {vol.email}
                    </p>
                  </div>
                </div>
              ...
              <button
                type="button"
                className="mobile-volunteer-toggle"
                aria-expanded={detailsExpanded}
                aria-controls={detailsId}
                aria-label={`${detailsExpanded ? 'Ocultar' : 'Mostrar'} detalhes de ${vol.name}`}
                onClick={() => toggleVolunteerDetails(vol.id)}
              >
                <span>{detailsExpanded ? 'Ocultar detalhes' : 'Turnos e proficiências'}</span>
                <ChevronDown size={18} aria-hidden="true" />
              </button>

              <div
                id={detailsId}
                className={`volunteer-card-details ${detailsExpanded ? 'mobile-expanded' : ''}`}
              >
                        ...
                        <div className="prof-rating-selector">
                          {[1, 2, 3].map((lvl) => (
                            <button
                              ...
                              onClick={() =>
                                onUpdateProficiency(vol.id, role.id, currentLevel === lvl ? 0 : lvl)
                              }
                              disabled={disabled}
                              title={
                                currentLevel === lvl
                                  ? `Remover proficiência de ${role.name}`
                                  : `Definir Nível ${lvl} para ${role.name}`
                              }
                            >
                              N{lvl}
                            </button>
      ...
      {/* Modal Add Volunteer */}
      {isAddModalOpen && (
        <Modal ariaLabelledBy="volunteer-modal-title" onClose={() => setIsAddModalOpen(false)}>
          <div className="modal-header">
            <h3 id="volunteer-modal-title">
              {editingVolunteerId ? 'Editar Voluntário' : 'Cadastrar Novo Voluntário'}
            </h3>
            <button
              type="button"
              className="close-btn"
