...
export const ScheduleMatrix = ({
  ...
  const getRoleIcon = (roleId) => {
    switch (roleId) {
      case 'COORDINATOR':
        return <Tv size={16} className="role-icon" />;
      case 'FIXED_CAM':
        return <Video size={16} className="role-icon" />;
      case 'JIB':
        return <Maximize2 size={16} className="role-icon" />;
      case 'SWITCHER':
        return <Sliders size={16} className="role-icon" />;
      case 'VMIX':
        return <Monitor size={16} className="role-icon" />;
      case 'FREEHAND':
        return <Mic size={16} className="role-icon" />;
      default:
        return <Video size={16} className="role-icon" />;
    }
  };
    ...
    <section className="matrix-container glass-panel">
        ...
        <div className="matrix-toolbar">
          ...
          {onGenerateAuto && (
            <button
              ...
              disabled={readOnly}
              title="Preencher ou recalcular a proposta pelo gerador do backend"
            >
              <Sparkles size={16} />
              Gerar Escala (IA)
            </button>
          ...
          {viewMode === 'grid' && (
            <div className="matrix-legend">
              ...
              <div className="legend-item">
                <span className="legend-badge level-1"></span>N1 (Treinando)
              </div>
              <div className="legend-item">
                <span className="legend-badge level-2"></span>N2 (Apto)
              </div>
              <div className="legend-item">
                <span className="legend-badge level-3"></span>N3 (Sênior)
              </div>
            </div>
          )}
        </div>
      ...
      {viewMode === 'table' && (
        <div className="app-table-container">
          <div className="app-banner-header">ESCALA COMPLETA TRANSMISSÃO</div>
                ...
                return (
                  <React.Fragment key={sunday.date}>
                    {shiftList.map((shiftItem) => (
                      <tr key={shiftItem.id}>
                        <td className="td-date-shift">
                          {shortDate} - {shiftItem.label}
                        </td>
                        {roles.map((role) => {
                          const { main: currentVolId, trainee: currentTraineeId } = getSlotAssignment(
                            schedule,
                            sunday.date,
                            shiftItem.id,
                            role.id
                          ...
                          const traineeUnavailable = isUnavailable(
                            currentTraineeId,
                            sunday.date,
                            shiftItem.id
                          );
                          ...
                          return (
                            <td
                              key={role.id}
                              className={`td-volunteer-cell ${mainUnavailable || mainDoubleBooked || traineeUnavailable ? 'has-conflict' : ''}`}
                            >
                              <div
                                style={{
                                  display: 'flex',
                              ...
                              >
                                ...
                                >
                                  <button
                                    type="button"
                                    onClick={() =>
                                      onToggleLockSlot && onToggleLockSlot(sunday.date, shiftItem.id, role.id)
                                    }
                                    disabled={readOnly}
                                    aria-label={
                                      isLocked
                                        ? `Destravar vaga de ${role.name} em ${sunday.formatted} no turno ${shiftItem.label}`
                                        : `Travar vaga de ${role.name} em ${sunday.formatted} no turno ${shiftItem.label}`
                                    }
                                    className={`slot-lock-button ${isLocked ? 'locked' : ''}`}
                                    title={
                                      isLocked
                                        ? 'Vaga travada (fixa durante a geração automática)'
                                        : 'Vaga livre (clique para travar)'
                                    }
                                  >
                                  ...
                                  >
                                    ...
                                    {volunteers
                                      .filter(
                                        (v) =>
                                          String(v.id) === String(currentVolId) ||
                                          (v.active && Number(v.proficiencies?.[role.id] || 0) >= 2)
                                      )
                                      .map((v) => {
                                  ...
                                  {profLevel > 0 && (
                                    <span
                                      className={`proficiency-pill level-${profLevel}`}
                                ...
                                >
                                  <GraduationCap
                                    size={13}
                                    style={{ color: canHaveTrainee ? '#38bdf8' : 'var(--text-dim)' }}
                                  />
                                  <select
                                    className="volunteer-select-table"
                                    style={{
                                    ...
                                    title={
                                      canHaveTrainee
                                        ? 'Selecione um voluntário N1 para treinar'
                                        : 'Treinamento requer mentor N3'
                                    }
                                  >
                                    <option value="">
                                      -- {canHaveTrainee ? 'Sem Treinando' : 'Requer mentor N3'} --
                                    </option>
                                    {volunteers
                                      .filter(
                                        (v) =>
                                          String(v.id) === String(currentTraineeId) ||
                                          (v.active && Number(v.proficiencies?.[role.id] || 0) === 1)
                                      )
                                      .map((v) => {
                                        ...
                                        return (
                                          <option key={v.id} value={v.id}>
                                            🎓 {v.name} (N{vLevel}){vLevel === 1 ? ' [Treinando]' : ''}
                                            {vUnavail ? ' ⚠️' : ''}
                                          </option>
                                        );
                                      })}
                          ...
                          >
                            <div className="role-slot-header">
                              <div className="role-info">
                                <button
                                  type="button"
                                  onClick={() =>
                                    onToggleLockSlot && onToggleLockSlot(sunday.date, shift.id, role.id)
                                  }
                                  disabled={readOnly}
                                  aria-label={
                                    isLocked
                                      ? `Destravar vaga de ${role.name} em ${sunday.formatted} no turno ${shift.name}`
                                      : `Travar vaga de ${role.name} em ${sunday.formatted} no turno ${shift.name}`
                                  }
                                  className={`slot-lock-button ${isLocked ? 'locked' : ''}`}
                                  title={
                                    isLocked
                              ...
                              {profLevel > 0 && (
                                <span className={`proficiency-pill level-${profLevel}`}>Op N{profLevel}</span>
                              )}
                            </div>
                            ...
                            <div className="volunteer-select-container">
                              <select
                                ...
                                value={currentVolId}
                                onChange={(e) =>
                                  onScheduleChange(sunday.date, shift.id, role.id, e.target.value, 'main')
                                }
                                disabled={readOnly}
                              >
                            ...
                            <div className="volunteer-select-container" style={{ gap: '0.4rem' }}>
                              <GraduationCap
                                size={15}
                                style={{
                                  color: canHaveTrainee ? '#38bdf8' : 'var(--text-dim)',
                                  flexShrink: 0
                                }}
                              />
                              <select
                                ...
                                disabled={readOnly || !canHaveTrainee}
                                onChange={(e) =>
                                  onScheduleChange(sunday.date, shift.id, role.id, e.target.value, 'trainee')
                                }
                              >
                                <option value="">
                                  -- {canHaveTrainee ? 'Selecionar Treinando (N1)' : 'Requer mentor N3'} --
                                </option>
                                {volunteers
                                  .filter(
                                    (v) =>
