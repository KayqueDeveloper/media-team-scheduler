...
export const getSundaysForMonth = (year: number, monthIndex: number) => {
  const sundays: Array<{ date: string; formatted: string; label: string }> = [];
...
type InitialShiftSchedule = Record<string, string>;
type InitialDaySchedule = { MORNING: InitialShiftSchedule; NIGHT: InitialShiftSchedule };

export const generateInitialSchedule = (sundaysList: readonly { date: string }[] = INITIAL_SUNDAYS) => {
  const schedule: Record<string, InitialDaySchedule> = {};
