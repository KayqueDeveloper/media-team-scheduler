// @ts-nocheck -- Legacy scheduling implementation behind the typed generator facade.
/**
 * Church Broadcast Scheduling System - Constraint Solver Engine
 * Original file: server/solver/scheduler.ts
 */
...
export function generateSchedule({
  ...
  const normalizedVolunteers = volunteers
    ...
    .map((volunteer) => {
      ...
      const oneAllowedShift =
        Array.isArray(volunteer.shifts) && volunteer.shifts.length === 1 ? volunteer.shifts[0] : null;
  ...
  volunteers.forEach((volunteer) => {
    ...
    if (!profMap.has(volunteerId) || !volunteer.proficiencies || typeof volunteer.proficiencies !== 'object')
      return;
  ...
  pastAssignments.forEach((assignment) => {
    const volunteerId = String(assignment.volunteerId ?? assignment.volunteer_id ?? assignment.vId ?? '');
  ...
  lockedAssignments.forEach((originalAssignment) => {
    ...
    if (!slot) {
      rejectLockedAssignment(
        assignment,
        'UNKNOWN_SLOT',
        'a vaga não pertence ao mês, turno ou função solicitados'
      );
    } else if (!volunteer) {
      ...
      rejectLockedAssignment(
        ...
        'uma alocação principal exige proficiência N2 ou N3'
      );
      ...
      rejectLockedAssignment(
        assignment,
        'MULTIPLE_SHIFTS_SAME_SUNDAY',
        'o voluntário já possui uma alocação travada nesse domingo'
      );
    } else if ((lockedVolunteerCounts.get(assignment.volunteerId) || 0) >= dynamicMonthlyLimit) {
      rejectLockedAssignment(
        assignment,
        'MONTHLY_LIMIT_EXCEEDED',
        'a alocação excede o limite mensal do voluntário'
      );
    } else {
  ...
  const attemptDefinitions = [
    ...
    ...(dynamicMonthlyLimit === 3 ? [{ monthlyLimit: 3, allowConsecutiveSundays: false }] : []),
    ...(dynamicMonthlyLimit === 3 ? [{ monthlyLimit: 3, allowConsecutiveSundays: true }] : [])
  ];
  ...
  schedule.forEach((principal) => {
    ...
    const candidates = normalizedVolunteers
      ...
      .filter((volunteerId) => {
        ...
        if (sundayIndexes.has(principal.sundayIndex - 1) || sundayIndexes.has(principal.sundayIndex + 1))
          return false;
        return true;
      })
  ...
  const warnings = invalidLockedAssignments.map(
    ({ assignment, reason }) =>
  ...
  if (selectedAttempt.monthlyLimit === 3) {
    warnings.push(
      'O limite de três participações foi necessário para aumentar a cobertura do mês com cinco domingos.'
    );
  }
  ...
  const seniorityBalancedShifts = sundays.reduce(
    (balancedCount, date) =>
      balancedCount +
